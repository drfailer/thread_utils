...
void graph_hadamard(Matrix &A, Matrix &B, Matrix &C, size_t tile_size) {
    ...
    for (size_t i = 0; i < C.rows; i += tile_size) {
        for (size_t j = 0; j < C.cols; j += tile_size) {
            ...
            tu_graph_push_task(&graph, 0, [](TU_Graph *, void *, void *rawdata, tu_i64) {
...
void graph_dgemm(Matrix &A, Matrix &B, Matrix &C, size_t tile_size) {
    ...
    std::function<void(TU_Graph*, void*, void*, tu_i64)> split_task, product_task,
    ...
    split_task = [&](TU_Graph *graph, void *, void *rawdata, tu_i64 type) {
        ...
        for (size_t i = 0; i < M->rows; i += tile_size) {
            for (size_t j = 0; j < M->cols; j += tile_size) {
                ...
                if (matrix_kind == MatrixKind::C) {
                    tu_graph_push_state(graph, sum_state_group, &sum_state_ctx, &run_lambda,
                            &sum_state, &tiles_mem[type].back(), type);
                } else {
                    tu_graph_push_state(graph, product_state_group, &product_state_cxt, &run_lambda,
    ...
    product_task = [&](TU_Graph *graph, void *, void *rawdata, tu_i64) {
        ...
        tu_graph_push_state(graph, sum_state_group, &sum_state_ctx, &run_lambda, &sum_state,
                p, (size_t)MatrixKind::P);
    };

    sum_task = [&](TU_Graph *graph, void *, void *rawdata, tu_i64) {
        ...
        deallocate_tile(p);
        tu_graph_push_state(graph, sum_state_group, &sum_state_ctx, &run_lambda, &sum_state, c, (size_t)MatrixKind::C);
    };

    std::vector<MatrixTile*> A_tiles(TM * TK);
    std::vector<MatrixTile*> B_tiles(TK * TN);
    product_state = [&](TU_Graph *graph, void *, void *rawdata, tu_i64 type) {
        ...
        case MatrixKind::A: {
            ...
            for (size_t col = 0; col < TN; ++col) {
                if (B_tiles[tile->col * TN + col] != nullptr) {
                    ...
                    tu_graph_push_task(graph, product_task_group, &run_lambda, &product_task, tiles, -1);
                }
            }
        } break;
        case MatrixKind::B: {
    ...
    sum_state = [&](TU_Graph *graph, void *, void *rawdata, tu_i64 type) {
        auto tile = (MatrixTile*)rawdata;
        auto matrix_kind = (MatrixKind)type;
        ...
        case MatrixKind::C: {
            ...
            if (sum_queues[c_idx].size() > 0) {
                ...
                tu_graph_push_task(graph, sum_task_group, &run_lambda, &sum_task, tiles, -1);
            } else {
                C_tiles[c_idx] = tile;
            }
        } break;
    ...
    tu_graph_push_task(&graph, split_task_group, &run_lambda, &split_task, &A, (tu_i64)MatrixKind::A);
    tu_graph_push_task(&graph, split_task_group, &run_lambda, &split_task, &B, (tu_i64)MatrixKind::B);
    tu_graph_push_task(&graph, split_task_group, &run_lambda, &split_task, &C, (tu_i64)MatrixKind::C);
